# LangGraph 멀티에이전트 이벤트 처리 시스템

LangGraph를 사용한 멀티에이전트 구조로 구현된 이벤트 자동 등록 시스템입니다.

## 🏗️ 아키텍처

### 멀티에이전트 구조

```
┌─────────────────────────────────────────────────────────────┐
│                    LangGraph Workflow                        │
│                                                               │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐  │
│  │   Sheets     │ →  │    Parser    │ →  │   Calendar   │  │
│  │    Agent     │    │    Agent     │    │    Agent     │  │
│  │              │    │  (LLM기반)   │    │              │  │
│  └──────────────┘    └──────────────┘    └──────────────┘  │
│         ↓                   ↓                    ↓          │
│    시트 읽기/쓰기        텍스트 파싱          캘린더 등록   │
└─────────────────────────────────────────────────────────────┘
```

### 3개의 전문 에이전트

1. **Sheets Agent** (`sheets_agent.py`)
   - 구글 시트에서 미처리 이벤트 읽기
   - 파싱 결과를 시트에 작성
   - 처리 상태 업데이트

2. **Parser Agent** (`parser_agent.py`)
   - LLM으로 텍스트 파싱
   - 데이터 검증
   - 형식 확인

3. **Calendar Agent** (`calendar_agent.py`)
   - 구글 캘린더에 일정 등록
   - 알림 설정
   - 이벤트 ID 관리

## 🔄 워크플로우

```
1. fetch_events
   ↓
2. select_next_event
   ↓
3. parse_event (Parser Agent)
   ↓
4. validate_data (Parser Agent)
   ↓
5. write_to_sheet (Sheets Agent)
   ↓
6. create_calendar_event (Calendar Agent)
   ↓
7. mark_synced (Sheets Agent)
   ↓
8. check_complete
   ↓
   모든 이벤트 처리 완료?
   - Yes → END
   - No → 2번으로 돌아가기
```

## 📁 프로젝트 구조

```
event-calendar-agent-langgraph/
├── agents/
│   ├── state.py               # 워크플로우 상태 정의
│   ├── sheets_agent.py        # 구글 시트 에이전트
│   ├── parser_agent.py        # 텍스트 파싱 에이전트
│   └── calendar_agent.py      # 캘린더 에이전트
│
├── workflow.py                # LangGraph 워크플로우 정의
├── main.py                    # 메인 실행 스크립트
├── requirements.txt           # 패키지 의존성
├── .env.example               # 환경 변수 템플릿
└── README.md                  # 이 파일
```

## 🚀 설치 및 실행

### 1. 패키지 설치

```bash
cd event-calendar-agent-langgraph

# 가상환경 생성
python -m venv venv
source venv/bin/activate  # Windows: venv\Scripts\activate

# 패키지 설치
pip install -r requirements.txt
```

### 2. 환경 변수 설정

```bash
cp .env.example .env
```

`.env` 파일 수정:
```env
OPENAI_API_KEY=sk-proj-여기에_실제_API_키
GOOGLE_SHEET_ID=여기에_시트_ID
GOOGLE_SHEET_NAME=Sheet1
GOOGLE_CALENDAR_ID=primary
```

### 3. OAuth 인증 설정

기존 프로젝트의 `oauth_credentials.json`과 `token.pickle` 파일을 재사용하거나,
새로 설정합니다 (상세 방법은 기존 프로젝트의 `SETUP_OAUTH.md` 참조).

### 4. 실행

```bash
python main.py
```

## 💡 기존 프로젝트와의 차이점

### 기존 (단순 순차 실행)

```python
# main.py
for event in events:
    parsed = parser.parse(event)
    sheets.write(parsed)
    calendar.create(parsed)
```

- 단순한 순차 처리
- 에러 처리 제한적
- 확장성 낮음

### LangGraph 버전 (멀티에이전트)

```python
# workflow.py
workflow = StateGraph(AgentState)
workflow.add_node("parse", parser_agent.parse)
workflow.add_node("write", sheets_agent.write)
workflow.add_node("calendar", calendar_agent.create)
workflow.add_conditional_edges(...)
```

- ✅ 에이전트 간 명확한 역할 분리
- ✅ 상태 관리 및 추적
- ✅ 에러 처리 및 재시도 가능
- ✅ 조건부 분기 및 복잡한 흐름 구현
- ✅ 확장성 높음 (새 에이전트 추가 쉬움)
- ✅ 디버깅 및 모니터링 용이

## 🎯 LangGraph의 장점

### 1. 상태 관리

```python
class AgentState(TypedDict):
    unprocessed_events: List[tuple]
    current_event: Optional[dict]
    processed_events: Annotated[List[EventInfo], add]
    messages: Annotated[List[str], add]
    errors: Annotated[List[dict], add]
    # ...
```

- 전역 상태를 통한 에이전트 간 데이터 공유
- 처리 과정 추적
- 에러 누적 및 분석

### 2. 조건부 분기

```python
def should_continue(state):
    if state.get("unprocessed_events"):
        return "select_next_event"
    else:
        return "end"

workflow.add_conditional_edges(
    "check_complete",
    should_continue,
    {
        "select_next_event": "select_next_event",
        "end": END
    }
)
```

- 동적 워크플로우 제어
- 상황에 따른 다른 경로 실행

### 3. 에이전트 협업

각 에이전트가 독립적으로 동작하면서도 공유 상태를 통해 협업:

```python
# Parser Agent
result = parser_agent.parse_event_text(state)
# → state["current_event"] 업데이트

# Calendar Agent (Parser의 결과 사용)
calendar_agent.create_calendar_event(state)
# → state["current_event"]["calendar_event_id"] 업데이트
```

### 4. 확장 가능성

새로운 에이전트 추가 예시:

```python
# 알림 에이전트 추가
class NotificationAgent:
    def send_notification(self, state):
        # 슬랙/이메일 알림 발송
        pass

workflow.add_node("notify", notification_agent.send_notification)
workflow.add_edge("mark_synced", "notify")
```

## 📊 실행 예시

```bash
$ python main.py

============================================================
🤖 LangGraph 멀티에이전트 이벤트 처리 시작
============================================================

📊 [Sheets Agent] 구글 시트에서 미처리 이벤트 읽기...
✅ 구글 시트에서 2개 이벤트 로드

============================================================
[1/2] 행 2 처리 시작
============================================================

🤖 [Parser Agent] 행 2 텍스트 파싱 중...
텍스트 미리보기: 연세자연애치과 안녕하세요. 강수혁님의 예약은 11월22일(토)...
  📌 제목: 연세자연애치과 예약
  📅 날짜: 2025-11-22
  🕐 시간: 12:30
  📍 장소: 서울특별시 서초구...

✓ [Parser Agent] 행 2 데이터 검증 중...
  ✅ 검증 통과

📝 [Sheets Agent] 행 2에 파싱 결과 작성 중...
✅ 행 2에 파싱 결과 작성 완료

📅 [Calendar Agent] 행 2 캘린더 등록 중...
✅ 캘린더에 이벤트 등록 완료!
   제목: 연세자연애치과 예약
   일시: 2025-11-22 12:30

============================================================
✅ 전체 처리 완료!
============================================================

📊 처리 결과:
  - 총 처리: 2개
  - 성공: 2개
  - 실패: 0개
```

## 🔍 디버깅

### 상태 확인

```python
# workflow.py 또는 main.py에서
result = app.invoke(initial_state)
print(result)  # 최종 상태 출력
```

### 로그 추적

각 에이전트의 `print()` 출력으로 진행 상황 추적:

```
📊 [Sheets Agent] ...
🤖 [Parser Agent] ...
📅 [Calendar Agent] ...
```

## 🎓 스터디 활용

### 학습 포인트

1. **LangGraph 기본 개념**
   - StateGraph
   - 노드와 엣지
   - 조건부 분기

2. **멀티에이전트 설계**
   - 에이전트 역할 분리
   - 상태 공유
   - 협업 패턴

3. **실전 응용**
   - 기존 코드와 LangGraph 통합
   - 에러 처리 및 재시도
   - 확장 가능한 구조 설계

### 실습 과제

1. **새 에이전트 추가**
   - 알림 에이전트 (슬랙/이메일)
   - 로깅 에이전트 (데이터베이스 저장)
   - 검증 강화 에이전트

2. **워크플로우 개선**
   - 에러 발생 시 재시도 로직
   - 병렬 처리 (여러 이벤트 동시 처리)
   - 우선순위 기반 처리

3. **시각화**
   - LangGraph의 시각화 기능 활용
   - 워크플로우 다이어그램 생성

## 📚 참고 자료

- [LangGraph 공식 문서](https://python.langchain.com/docs/langgraph)
- [LangChain 공식 문서](https://python.langchain.com/)
- [멀티에이전트 시스템 패턴](https://python.langchain.com/docs/use_cases/agent_simulations/)

## 🆚 비교표

| 특징 | 기존 버전 | LangGraph 버전 |
|------|----------|---------------|
| 구조 | 순차 실행 | 멀티에이전트 |
| 상태 관리 | 없음 | TypedDict로 명확한 상태 |
| 에러 처리 | 제한적 | 상태 추적 및 누적 |
| 확장성 | 낮음 | 높음 (에이전트 추가 쉬움) |
| 디버깅 | 어려움 | 쉬움 (상태 추적) |
| 복잡한 흐름 | 구현 어려움 | 조건부 분기로 쉽게 구현 |
| 학습 곡선 | 낮음 | 중간 |

## 라이선스

MIT License
